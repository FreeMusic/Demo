...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    for (id layer in self.view.layer.sublayers) {
        if([layer isKindOfClass:[SXYAnimationLayer class]])
        {
            [layer removeFromSuperlayer];
        }
    }
    [SXYAnimationLayer createAnimationLayerByString:@"某某，我爱你" andRect:CGRectMake(0, 100, self.view.frame.size.width, self.view.frame.size.width) andView:self.view andFont:[UIFont boldSystemFontOfSize:40] andStrokeColor:[UIColor cyanColor]];
}
