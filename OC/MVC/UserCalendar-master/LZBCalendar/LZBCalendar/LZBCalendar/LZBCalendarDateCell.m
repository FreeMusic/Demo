...
- (void)layoutSubviews
{
    ...
    {
        self.dateLabel.center = CGPointMake(width*0.5, self.dateLableHeight);
        self.dateLabel.bounds = CGRectMake((width-40)/2, self.dateLableHeight, 40, 40);
    }
...
- (void)upDateTextLabelBackGroudColorAnddidChangeColor:(BOOL)changeColor{
    if (changeColor == NO) {
        [self addAnimationToView:self];
    }
    self.dateLabel.backgroundColor = (changeColor == YES) ? [UIColor clearColor] : [UIColor yellowColor];
}
