...
@property (nonatomic, assign) NSIndexPath *indexPath;//记录上次用户点击了哪个日期
...
- (void)setupUI
{
    ...
    [self.headerView setPreviousMonthClick:^(NSDate *date) {
        ...
        //将非选中状态的cell置为了透明色
        LZBCalendarDateCell *normalCell = (LZBCalendarDateCell *)[weakSelf.collectionView cellForItemAtIndexPath:weakSelf.indexPath];
        [normalCell upDateTextLabelBackGroudColorAnddidChangeColor:YES];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row >=marginDays && indexPath.row <=marginDays + daysThisMonth - 1)
    {
        ...
        //改变选中的背景颜色
        [cell upDateTextLabelBackGroudColorAnddidChangeColor:NO];
        //将非选中状态的cell置为了透明色
        LZBCalendarDateCell *normalCell = (LZBCalendarDateCell *)[collectionView cellForItemAtIndexPath:self.indexPath];
